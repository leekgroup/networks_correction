---
title: "Significance of improved precision without significant loss in recall -- glasso networks using canonical pathway databases"
output:
  html_document:
    code_folding: hide
---
...
## load data
load("/home-3/pparsan1@jhu.edu/work2/princy/networks_correction/publication_figures/fig3/fig3.RData")
	fig3 <- list(blood = plot.blood, lung = plot.lung, muscle = plot.muscle, sub = plot.sub, thyroid = plot.thyroid)

## Precision and Recall ( Uncorrected vs PC corrected)
	precision <- lapply(fig3, function(x){
		eachtissue <- split(x, x$type)
		tmp.pval <- wilcox.test(eachtissue[[1]]$precision, eachtissue[[8]]$precision, alternative = "greater")$p.value
		})

	recall <- lapply(fig3, function(x){
		eachtissue <- split(x, x$type)
