---
title: "Precision and recall in WGCNA networks using canonical pathway databases with RIN, GC content and exonic rate as known covariates as predictors"
output:
  html_document:
    code_folding: hide
---
...
plot.thyroid <- plot.thyroid[-which(plot.thyroid$type %in% c("RIN","expeff", "gene GC%", "exonic rate", "multicorr")),]
plot.thyroid$type <- factor(plot.thyroid$type, levels = c("PC corrected","half-PC","quarter-PC", "multi3", "uncorrected"))
...
plot.muscle <- plot.muscle[-which(plot.muscle$type %in% c("RIN","expeff", "gene GC%", "exonic rate", "multicorr")),]
plot.muscle$type <- factor(plot.muscle$type, levels = c("PC corrected","half-PC","quarter-PC", "multi3", "uncorrected"))
...
plot.lung <- plot.lung[-which(plot.lung$type %in% c("RIN","expeff", "gene GC%", "exonic rate", "multicorr")),]
plot.lung$type <- factor(plot.lung$type, levels = c("PC corrected","half-PC","quarter-PC", "multi3", "uncorrected"))
...
plot.blood <- plot.blood[-which(plot.blood$type %in% c("RIN","expeff", "gene GC%", "exonic rate", "multicorr")),]
plot.blood$type <- factor(plot.blood$type, levels = c("PC corrected","half-PC","quarter-PC", "multi3", "uncorrected"))
...
plot.sub <- plot.sub[-which(plot.sub$type %in% c("RIN","expeff", "gene GC%", "exonic rate", "multicorr")),]
plot.sub$type <- factor(plot.sub$type, levels = c("PC corrected","half-PC","quarter-PC", "multi3", "uncorrected"))
