...
plot_edgecounts <- function(tissuename, genes.network.index, gene.symbols.innetwork ){
	fn <- tissuename
	# fn <- "Subcutaneous"
	version.data <- c(pc, half.pc, quarter.pc, exonic, expeff, rin, raw)

	edgecounts.network <- vector("list", length = 7)
	for(j in 1:length(version.data)){
		...
		edgecounts.network[[j]] <- lapply(wgcna.networks, function(each.network, genes.index, genes.withsymbols){
			...
			networks.list <- do.call(rbind, networks.list)
			networks.list <- unique(data.frame(networks.list))
			networks.list <- graph.data.frame(networks.list, directed = FALSE)
			return(ecount(networks.list))
...
#	names(edgecounts.network) <- c("PC-corrected", "half-PC", "quarter-PC", "RIN corrected", "Uncorrected")
	...
	colnames(edgecounts.network) <- c("PC-corrected", "half-PC", "quarter-PC", "exonic rate", "expeff", "RIN corrected", "uncorrected")
...
save(plot.wgcna.thyroid, plot.wgcna.muscle, plot.wgcna.lung, plot.wgcna.blood, plot.wgcna.sub, file = "fig4-wgcna.RData")
