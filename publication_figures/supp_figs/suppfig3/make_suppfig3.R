...
supp_fig3 <- plot_grid(plot.sub + xlim(0,0.01) + ylim(0, 0.7) + theme(legend.position="none"),
	plot.thyroid + xlim(0,0.01) + ylim(0, 0.7) +  theme(legend.position="none"),
	plot.lung + xlim(0,0.01) + ylim(0, 0.7) + theme(legend.position="none"),
	plot.muscle + xlim(0,0.01) + ylim(0, 0.7) + theme(legend.position="none"),
	plot.blood + xlim(0,0.01) + ylim(0, 0.7) + theme(legend.position="none"),
	legend,
           labels = c("a", "b", "c", "d", "e", ""),
           hjust = -1,
           nrow = 3
...
#supp_fig3 <- plot_grid( supp_fig3, legend, rel_widths = c(3, .4))

pdf("supp_fig3.pdf", height = 7.2, width = 7.2)
print(supp_fig3)
dev.off()
