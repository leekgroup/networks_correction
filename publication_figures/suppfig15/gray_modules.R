...
grayraw <- sapply(modraw, function(x) sapply(x, function(y) y[1]))
graypc <- sapply(modpc, function(x) sapply(x, function(y) y[1]))
...
ggplot(graymerged, aes(x = cutheights, y = value, col = Type, shape = Tissue))+
  geom_line(aes(linetype= Type))+
  geom_point()+
  xlab("Cut-heights")+
  ylab("# of genes with unassigned module")+
  theme_classic()

ggsave(filename = "num_genes_gray_modules.png", width = 5, height = 4, units = "in", dpi = 600)
