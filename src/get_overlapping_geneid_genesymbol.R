...
library(dplyr)
d = read.delim("/work-zfs/abattle4/parsana/networks_correction_coomon_genes/data/etc/gencode_exon_utr_position_bygene.txt", header = F, stringsAsFactors = F)
...
write.table(ensembl_with_some_overlap, file = "/work-zfs/abattle4/parsana/networks_correction_coomon_genes/data/etc/ensembl_ids_overlapping_genes.txt", quote = F, row.names = F)
...
write.table(gsymbol_with_some_overlap, file = "/work-zfs/abattle4/parsana/networks_correction_coomon_genes/data/etc/gene_symbol_overlapping_genes.txt", quote = F, row.names = F)
